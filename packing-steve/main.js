...
class ge {
  ...
  initRuntimePasses() {
    ...
    for (const s of r) {
      ...
      try {
        const l = he(e, pe, a), u = this.cacheUniformLocations(l, o.namedSamplers), d = G(e, this._width, this._height), E = G(e, this._width, this._height), y = me(e, d), T = {
          name: s,
...
class mt {
  ...
  show(e, t) {
    ...
    const n = e.filter((u) => u.isFromCommon), r = e.filter((u) => !u.isFromCommon), c = [...n.length > 0 ? [n[0]] : [], ...r].map(({ passName: u, error: d, isFromCommon: E, originalLine: y, lineMapping: T }) => {
...
`);
  ...
  return t.slice(r, s).map((a, c) => {
    const l = r + c + 1, u = l === e, d = String(l).padStart(4, " "), E = Ae(a);
...
class Fe {
  ...
  createVec2Pad(e, t) {
    ...
    const b = (f) => {
      ...
      let v = Math.max(0, Math.min(1, (w - g.left) / g.width)), C = Math.max(0, Math.min(1, (R - g.top) / g.height));
      const A = r[0] + v * (s[0] - r[0]), S = r[1] + (1 - C) * (s[1] - r[1]), B = [A, S];
      this.values[e] = B, E.style.left = `${v * 100}%`, E.style.top = `${C * 100}%`, u.textContent = this.formatVec2(B), this.onChange(e, B);
    }, x = (f) => {
...
function Tt(i, e) {
  ...
  const u = !!((f = i.script) != null && f.setup || (g = i.script) != null && g.onFrame), d = Object.entries(i.uniforms).filter(([, v]) => !F(v)), E = [];
  for (const [v, C] of d) {
    if (F(C))
      continue;
    const A = r[v] ?? C.value;
  ...
  const y = d.map(([v, C]) => `uniform ${C.type === "bool" ? "bool" : C.type} ${v};`).join(`
`), T = s.map((v) => `// Array uniform: ${v.name} (max ${v.count})
  ...
  constructor(e) {
    ...
    if (this.isMultiView) {
      ...
      for (const o of r.views) {
        ...
        const c = this.createViewProject(r, o), l = new ve({
          container: a,
          project: c,
          pixelRatio: t,
          viewNames: s
    ...
    !e.skipUniformsPanel && this.project.uniforms && Object.values(this.project.uniforms).some((r) => re(r)) && (this.uniformsPanel = new vt({
...
const ie = ["Image", "BufferA", "BufferB", "BufferC", "BufferD"], It = ["BufferA", "BufferB", "BufferC", "BufferD"], Le = ["iChannel0", "iChannel1", "iChannel2", "iChannel3"], Nt = "default", Pe = !0, Ot = "light", De = /* @__PURE__ */ new Set([
  ...
  "iPinchDelta",
  "iPinchCenter"
]), $t = /^[a-zA-Z_][a-zA-Z0-9_]*$/, Ht = /* @__PURE__ */ new Set([
  "attribute",
  "const",
  "uniform",
  "varying",
...
async function Wt(i, e, t) {
  ...
  return n ? (zt(n, e), n.mode === "shadertoy" ? Zt(i, e, n, t) : Jt(i, e, n, t)) : qt(i, e, t);
}
...
const rn = /* @__PURE__ */ Object.assign({ "./shaders/packing-steve/image.glsl": () => Promise.resolve().then(() => fn).then((i) => i.default) }), sn = /* @__PURE__ */ Object.assign({ "./shaders/packing-steve/config.json": () => Promise.resolve().then(() => gn).then((i) => i.default) }), on = /* @__PURE__ */ Object.assign({}), an = /* @__PURE__ */ Object.assign({});
let ne = null;
async function cn() {
  return ne || (ne = await tn("shaders/packing-steve", rn, sn, on, an)), ne;
}
    ...
    var r = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, s = 0, o = {}, a = {
      ...
      languages: {
        ...
        insertBefore: function(m, h, p, f) {
          ...
          var C = f[m];
          return f[m] = w, a.languages.DFS(a.languages, function(A, S) {
            S === C && A != m && (this[A] = w);
          }), w;
        },
        ...
        DFS: function m(h, p, f, g) {
          ...
          for (var R in h)
            if (h.hasOwnProperty(R)) {
              p.call(h, R, h[R], f || R);
              var v = h[R], C = a.util.type(v);
              C === "Object" && !g[w(v)] ? (g[w(v)] = !0, m(v, p, null, g)) : C === "Array" && !g[w(v)] && (g[w(v)] = !0, m(v, p, R, g));
            }
        }
      },
      plugins: {},
      ...
      for (var R in p)
        if (!(!p.hasOwnProperty(R) || !p[R])) {
          ...
          for (var C = 0; C < v.length; ++C) {
            if (w && w.cause == R + "," + C)
              return;
            var A = v[C], S = A.inside, B = !!A.lookbehind, ce = !!A.greedy, Ve = A.alias;
            if (ce && !A.pattern.global) {
              var ze = A.pattern.toString().match(/[imsuy]*$/)[0];
              A.pattern = RegExp(A.pattern.source, ze + "g");
            }
            for (var le = A.pattern || A, k = f.next, P = g; k !== h.tail && !(w && P >= w.reach); P += k.value.length, k = k.next) {
              var I = k.value;
              ...
              if (!(I instanceof c)) {
                ...
                if (ce) {
                  if (L = l(le, P, m, B), !L || L.index >= m.length)
                    break;
                  var z = L.index, Xe = L.index + L[0].length, M = P;
                  for (M += k.value.length; z >= M; )
                  ...
                  continue;
                var z = L.index, X = L[0], J = I.slice(0, z), ue = I.slice(z + X.length), Q = P + I.length;
                w && Q > w.reach && (w.reach = Q);
                var K = k.prev;
                J && (K = E(h, K, J), P += J.length), y(h, K, V);
                var Ke = new c(R, S ? a.tokenize(X, S) : X, Ve, X);
                if (k = E(h, K, Ke), ue && E(h, k, ue), V > 1) {
                  var ee = {
                    cause: R + "," + C,
                    reach: Q
                  };
      ...
      if (x.matches(d)) {
        ...
        f && f.loadLanguages(h), E(
          ...
          function(g) {
            ...
            if (w) {
              var R = g.split(/\r\n?|\n/g), v = w[0], C = w[1] == null ? R.length : w[1];
              v < 0 && (v += R.length), v = Math.max(0, Math.min(v - 1, R.length)), C < 0 && (C += R.length), C = Math.max(0, Math.min(C, R.length)), g = R.slice(v, C).join(`
...
`, dn.highlightElement(c);
    const y = E.split(`
`);
    r.innerHTML = y.map((T, b) => `<span>${b + 1}</span>`).join(""), t && t(E);
  }
  function u() {
...
`, fn = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: mn
}, Symbol.toStringTag, { value: "Module" })), He = "default", je = !0, pn = {
  layout: He,
  controls: je
