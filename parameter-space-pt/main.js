...
function me(i, e) {
  ...
  if (n !== i.FRAMEBUFFER_COMPLETE)
    throw i.deleteFramebuffer(t), new Error(`Framebuffer incomplete: ${it(i, n)}`);
...
`, ct = `// --- Keyboard helpers (auto-injected) ---
// Letter keys
...
`), t.viewNames && t.viewNames.length > 1) {
    ...
    n.push(""), t.namedSamplers.has("keyboard") && (n.push(ct), n.push(""));
...
class dt {
  ...
  updateAudioTexture(e) {
    var t;
    (t = this._audioTexture) != null && t.analyser && (this._audioTexture.analyser.getByteFrequencyData(this._audioTexture.frequencyData), this._audioTexture.analyser.getByteTimeDomainData(this._audioTexture.waveformData), et(e, this._audioTexture.texture, this._audioTexture.frequencyData, this._audioTexture.waveformData));
  }
  updateVideoTextures(e) {
    for (const t of this._videoTextures)
      !t.ready || !t.video || t.video.readyState < HTMLMediaElement.HAVE_CURRENT_DATA || (tt(e, t.texture, t.video), t.video.videoWidth > 0 && (t.width = t.video.videoWidth, t.height = t.video.videoHeight));
  }
  // ===========================================================================
  // Cleanup
  // ===========================================================================
...
class mt {
  constructor(e, t) {
    this._ubos = [], this._dirtyScalars = /* @__PURE__ */ new Set(), this._uniforms = t, this._store = new ht(t), this.initUBOs(e);
  ...
  set(e, t) {
    ...
    if (this._store.set(e, t), F(n)) {
      ...
      if (r) {
        ...
        const c = s.length / a, l = ot(n.type, c, s, r.paddedData);
        l !== r.paddedData && r.paddedData.set(l);
        const u = st(n.type, c), d = r.paddedData.length;
...
class ge {
  constructor(e) {
    ...
    this._keyboardTexture = {
      ...
      width: 256,
      height: 3
    }, this.initProjectTextures(), this._media = new dt(this.gl, e.project), this._uniformMgr = new mt(this.gl, e.project.uniforms), e.viewNames && e.viewNames.length > 1 && (this._viewNames = e.viewNames), this.initRuntimePasses();
  }
  // ===========================================================================
  ...
  updateTexture(e, t, n, r) {
    ...
    if (s && s.width === t && s.height === n && s.isFloat === o) {
      ...
      const a = nt(this.gl, (s == null ? void 0 : s.texture) ?? null, t, n, r);
   ...
   */
  initRuntimePasses() {
    const e = this.gl, t = this.project, n = We(e);
    this._sharedVAO = n;
    ...
    for (const s of r) {
      ...
      try {
        const l = he(e, fe, a), u = this.cacheUniformLocations(l, o.namedSamplers), d = G(e, this._width, this._height), v = G(e, this._width, this._height), y = me(e, d), T = {
          name: s,
...
`, ""), x = y;
      ...
      return {
        ...
        error: ft(_, T, v),
        codeContext: x !== null && m ? vt(m, x) : null
      };
...
`).map((n) => {
    ...
    if (r) {
      ...
      let c = a;
      return t && e.commonStartLine > 0 ? c = a - e.commonStartLine + 1 : e.userCodeStartLine > 0 && a >= e.userCodeStartLine && (c = a - e.userCodeStartLine + 1), `Line ${c}: ${gt(o)}`;
    }
    return n;
  }).join(`
`);
...
function Tt(i, e) {
  const t = yt(i, e), n = new Blob([t], { type: "text/html" }), s = `${i.root.split("/").pop() || "shader"}.html`, o = URL.createObjectURL(n), a = document.createElement("a");
...
class D {
  constructor(e) {
    ...
    if (this.recorder = new xt(this.primaryView.canvas, this.container, this.project.root), this.statsPanel = new wt(this.container), this.statsPanel.updateResolution(this.primaryView.canvas.width, this.primaryView.canvas.height), this.isMultiView) {
    ...
    if (this.project.controls && !e.skipPlaybackControls && (this.playbackControls = new _t(this.container, {
    ...
    !e.skipUniformsPanel && this.project.uniforms && Object.values(this.project.uniforms).some((r) => re(r)) && (this.uniformsPanel = new bt({
  ...
  openRenderDialog() {
    new Ct(this.container, this.primaryView.canvas.width, this.primaryView.canvas.height, (t) => this.renderOffline(t)).open();
  }
  renderOffline(e) {
    let t = !1;
    const n = () => {
...
class Ft {
  ...
  async buildEditorPanel() {
    try {
      const { EditorPanel: e } = await Promise.resolve().then(() => dn);
      if (this._disposed)
        return;
...
function Ut(i, e, t) {
  const n = e.views.map((a) => a.name), r = Dt(e.viewLayout, {
    container: i,
    project: e,
    viewNames: n
  }), s = new D({
...
const ie = ["Image", "BufferA", "BufferB", "BufferC", "BufferD"], Nt = ["BufferA", "BufferB", "BufferC", "BufferD"], Le = ["iChannel0", "iChannel1", "iChannel2", "iChannel3"], Ot = "default", Pe = !0, $t = "light", De = /* @__PURE__ */ new Set([
  "iResolution",
  "iTime",
  "iTimeDelta",
  "iFrame",
  ...
  "iPinchDelta",
  "iPinchCenter"
]), zt = /^[a-zA-Z_][a-zA-Z0-9_]*$/, Vt = /* @__PURE__ */ new Set([
  "attribute",
  "const",
  "uniform",
  "varying",
  ...
  for (const s of ie) {
    ...
    if (!(!o || typeof o != "object")) {
      for (const a of Object.keys(o))
        jt.has(a) || t.push(`Unknown key '${a}' in pass '${s}'`);
      for (const a of Le) {
        const c = o[a];
...
function W(i, e, t, n, r, s) {
  if (!i)
    return { kind: "none" };
  const o = ke(i);
  return o ? Wt(o, e, t, n, r, s) : { kind: "none" };
}
...
async function Zt(i, e, t, n) {
  ...
  for (const u of ie) {
    ...
    if (!d)
      continue;
    const v = d.source ?? It(u), y = i.joinPath(e, v);
    if (!await i.exists(y))
...
const en = /* @__PURE__ */ new Set(["keyboard", "audio", "webcam"]);
async function tn(i, e, t, n, r) {
  ...
  for (const [T, b] of Object.entries(t.textures ?? {}))
    ...
    } else en.has(b) || l.set(T, { kind: "script", name: b });
...
const sn = /* @__PURE__ */ Object.assign({ "./shaders/parameter-space-pt/common.glsl": () => Promise.resolve().then(() => fn).then((i) => i.default), "./shaders/parameter-space-pt/invertedStrip.glsl": () => Promise.resolve().then(() => vn).then((i) => i.default), "./shaders/parameter-space-pt/resulting_pack.glsl": () => Promise.resolve().then(() => bn).then((i) => i.default), "./shaders/parameter-space-pt/stripPack.glsl": () => Promise.resolve().then(() => yn).then((i) => i.default) }), on = /* @__PURE__ */ Object.assign({ "./shaders/parameter-space-pt/config.json": () => Promise.resolve().then(() => wn).then((i) => i.default) }), an = /* @__PURE__ */ Object.assign({}), cn = /* @__PURE__ */ Object.assign({ "./shaders/parameter-space-pt/script.js": () => Promise.resolve().then(() => _n) });
let ne = null;
async function ln() {
  return ne || (ne = await nn("shaders/parameter-space-pt", sn, on, an, cn)), ne;
}
      ...
      for (var R in f)
        if (!(!f.hasOwnProperty(R) || !f[R])) {
          ...
          for (var C = 0; C < E.length; ++C) {
            if (w && w.cause == R + "," + C)
              return;
            var A = E[C], S = A.inside, U = !!A.lookbehind, ce = !!A.greedy, je = A.alias;
            if (ce && !A.pattern.global) {
              var Xe = A.pattern.toString().match(/[imsuy]*$/)[0];
              A.pattern = RegExp(A.pattern.source, Xe + "g");
            }
            for (var le = A.pattern || A, k = p.next, P = g; k !== h.tail && !(w && P >= w.reach); P += k.value.length, k = k.next) {
              var I = k.value;
              ...
              if (!(I instanceof c)) {
                ...
                var K = k.prev;
                Z && (K = v(h, K, Z), P += Z.length), y(h, K, H);
                var Ge = new c(R, S ? a.tokenize(X, S) : X, je, X);
                if (k = v(h, K, Ge), ue && v(h, k, ue), H > 1) {
                  var ee = {
                    cause: R + "," + C,
                    reach: J
                  };
                  u(m, h, f, k.prev, P, ee), w && ee.reach > w.reach && (w.reach = ee.reach);
                }
              }
            }
          }
...
const $e = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  createEditor: mn
}, Symbol.toStringTag, { value: "Module" })), pn = `//constants
float EPS=1e-4;
...
`, fn = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: pn
}, Symbol.toStringTag, { value: "Module" })), gn = `vec3 palette(float t) {
...
`, vn = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: gn
}, Symbol.toStringTag, { value: "Module" })), En = `vec3 palette(float t) {
    vec3 a = vec3(0.5, 0.5, 0.5);
    vec3 b = vec3(0.5, 0.5, 0.5);
    vec3 c = vec3(1.0, 1.0, 1.0);
    vec3 d = vec3(0.00, 0.33, 0.67);
...
`, bn = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: En
}, Symbol.toStringTag, { value: "Module" })), Tn = `vec3 palette(float t) {
    vec3 a = vec3(0.5, 0.5, 0.5);
    vec3 b = vec3(0.5, 0.5, 0.5);
    vec3 c = vec3(1.0, 1.0, 1.0);
    vec3 d = vec3(0.00, 0.33, 0.67);
...
`, yn = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Tn
}, Symbol.toStringTag, { value: "Module" })), ze = "ACP parameter space", Ve = [
  "stripPack",
  "resulting_pack",
  "invertedStrip"
], He = "split", xn = {
