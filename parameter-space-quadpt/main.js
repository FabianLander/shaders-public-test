...
function me(i, e) {
  ...
  if (n !== i.FRAMEBUFFER_COMPLETE)
    throw i.deleteFramebuffer(t), new Error(`Framebuffer incomplete: ${ot(i, n)}`);
...
`, dt = `// --- Keyboard helpers (auto-injected) ---
// Letter keys
...
`), t.viewNames && t.viewNames.length > 1) {
    ...
    n.push(""), t.namedSamplers.has("keyboard") && (n.push(dt), n.push(""));
...
`), t.viewNames && t.viewNames.length > 1) {
  ...
  const r = Object.entries(t.uniforms).filter(([, d]) => !P(d));
  if (r.length > 0) {
    n.push("// Custom uniforms");
    for (const [d, v] of r) {
      const T = v.type === "bool" ? "bool" : v.type;
      n.push(`uniform ${T} ${d};`);
    }
    n.push("");
  }
...
class pt {
  ...
  updateAudioTexture(e) {
    var t;
    (t = this._audioTexture) != null && t.analyser && (this._audioTexture.analyser.getByteFrequencyData(this._audioTexture.frequencyData), this._audioTexture.analyser.getByteTimeDomainData(this._audioTexture.waveformData), it(e, this._audioTexture.texture, this._audioTexture.frequencyData, this._audioTexture.waveformData));
  }
  updateVideoTextures(e) {
    for (const t of this._videoTextures)
      !t.ready || !t.video || t.video.readyState < HTMLMediaElement.HAVE_CURRENT_DATA || (rt(e, t.texture, t.video), t.video.videoWidth > 0 && (t.width = t.video.videoWidth, t.height = t.video.videoHeight));
  }
  // ===========================================================================
  // Cleanup
  // ===========================================================================
...
class ft {
  ...
  initializeDefaults() {
    for (const [e, t] of Object.entries(this.definitions))
      P(t) ? this.values[e] = new Float32Array($(t.type, t.count)) : this.values[e] = this.cloneValue(t.value);
  }
  /**
  ...
  reset(e) {
    ...
    return t ? (P(t) ? this.values[e] = new Float32Array($(t.type, t.count)) : this.values[e] = this.cloneValue(t.value), !0) : !1;
  }
  /**
  ...
  getDefault(e) {
    const t = this.definitions[e];
    if (t)
      return P(t) ? new Float32Array($(t.type, t.count)) : this.cloneValue(t.value);
  }
  /**
...
class gt {
  constructor(e, t) {
    this._ubos = [], this._dirtyScalars = /* @__PURE__ */ new Set(), this._uniforms = t, this._store = new ft(t), this.initUBOs(e);
    for (const [n, r] of Object.entries(t))
      P(r) || this._dirtyScalars.add(n);
  }
  // ===========================================================================
  // Accessors
  // ===========================================================================
  ...
  set(e, t) {
    ...
    if (this._store.set(e, t), P(n)) {
      ...
      if (r) {
        ...
        const c = s.length / a, l = lt(n.type, c, s, r.paddedData);
        l !== r.paddedData && r.paddedData.set(l);
        const u = ct(n.type, c), d = r.paddedData.length;
...
class ge {
  ...
  updateTexture(e, t, n, r) {
    ...
    if (s && s.width === t && s.height === n && s.isFloat === o) {
      ...
      const a = st(this.gl, (s == null ? void 0 : s.texture) ?? null, t, n, r);
  ...
  step(e, t, n, r, s) {
    ...
    for (const v of d) {
      const T = this._passes.find((y) => y.name === v);
      T && (this.executePass(T, u), this.swapPassTextures(T));
    }
  ...
  resize(e, t) {
    ...
    for (const r of this._passes)
      n.deleteTexture(r.currentTexture), n.deleteTexture(r.previousTexture), n.deleteFramebuffer(r.framebuffer), r.currentTexture = G(n, e, t), r.previousTexture = G(n, e, t), r.framebuffer = me(n, r.currentTexture);
  }
  /**
  ...
  updateKeyboardTexture() {
    this._keyboardTexture && tt(this.gl, this._keyboardTexture.texture, this._keyStates, this._toggleStates);
  }
  /**
  ...
  cacheUniformLocations(e, t) {
    ...
    for (const [s, o] of Object.entries(this.project.uniforms))
      P(o) || r.set(s, n.getUniformLocation(e, s));
  ...
  initRuntimePasses() {
    ...
    for (const s of r) {
      ...
      try {
        const l = he(e, fe, a), u = this.cacheUniformLocations(l, o.namedSamplers), d = G(e, this._width, this._height), v = G(e, this._width, this._height), T = me(e, d), y = {
          name: s,
        ...
        if (d) {
          ...
          if (c.commonStartLine > 0 && c.commonLines > 0) {
            ...
            y >= c.commonStartLine && y <= b && (v = !0, T = y - c.commonStartLine + 1);
          }
          !v && c.userCodeStartLine > 0 && y >= c.userCodeStartLine && (T = y - c.userCodeStartLine + 1);
        }
...
class vt {
  ...
  show(e, t) {
    ...
    const n = e.filter((u) => u.isFromCommon), r = e.filter((u) => !u.isFromCommon), c = [...n.length > 0 ? [n[0]] : [], ...r].map(({ passName: u, error: d, isFromCommon: v, originalLine: T, lineMapping: y }) => {
      const b = d.replace(`Shader compilation failed:
`, ""), w = T;
      let _ = b;
      w !== null && (_ = b.replace(/ERROR:\s*\d+:(\d+):/g, `ERROR: 0:${w}:`));
      let m = null;
      if (v)
        m = t.commonSource;
      else {
...
`);
  ...
  return t.slice(r, s).map((a, c) => {
    const l = r + c + 1, u = l === e, d = String(l).padStart(4, " "), v = Pe(a);
...
class Ee {
  ...
  constructor(e) {
    ...
    }), this._resizeObserver.observe(this.container), this.input = new Tt(this.canvas, this._pixelRatio), this.input.onFirstGesture = () => this.initMediaOnGesture(), this.initVideoFiles();
  }
  // ===========================================================================
  // Per-Frame Rendering
  // ===========================================================================
...
class Ae {
  constructor(e) {
    ...
    for (const [n, r] of Object.entries(this.uniforms))
      P(r) || r.hidden || (this.values[n] = ((t = e.initialValues) == null ? void 0 : t[n]) ?? r.value);
    this.render();
  }
  /**
  ...
  createVec2Pad(e, t) {
    ...
    const T = (p) => {
      const g = (p[0] - r[0]) / (s[0] - r[0]) * 100, x = (1 - (p[1] - r[1]) / (s[1] - r[1])) * 100;
      v.style.left = `${g}%`, v.style.top = `${x}%`;
    };
    T(n);
    let y = !1;
    const b = (p) => {
      const g = d.getBoundingClientRect(), x = "touches" in p ? p.touches[0].clientX : p.clientX, R = "touches" in p ? p.touches[0].clientY : p.clientY;
      let E = Math.max(0, Math.min(1, (x - g.left) / g.width)), C = Math.max(0, Math.min(1, (R - g.top) / g.height));
      const k = r[0] + E * (s[0] - r[0]), S = r[1] + (1 - C) * (s[1] - r[1]), U = [k, S];
      this.values[e] = U, v.style.left = `${E * 100}%`, v.style.top = `${C * 100}%`, u.textContent = this.formatVec2(U), this.onChange(e, U);
    }, w = (p) => {
      y = !0, b(p), p.preventDefault();
    }, _ = (p) => {
      y && b(p);
    }, m = () => {
      y = !1;
    ...
    return d.addEventListener("touchstart", h), document.addEventListener("touchmove", f), document.addEventListener("touchend", m), this.documentListeners.push({ type: "touchmove", handler: f }), this.documentListeners.push({ type: "touchend", handler: m }), a.appendChild(c), a.appendChild(d), {
      ...
      update: (p) => {
        const g = p;
        T(g), u.textContent = this.formatVec2(g);
      }
    };
  }
  // ===========================================================================
  ...
  createColorPicker(e, t) {
    ...
    return d.className = "uniform-control-color-swatch", d.style.backgroundColor = this.rgbToHex(n), u.addEventListener("input", () => {
      ...
      element: s,
      update: (v) => {
        const T = this.rgbToHex(v);
        u.value = T, d.style.backgroundColor = T, c.textContent = T;
      }
    };
  }
  // ===========================================================================
  ...
  createColorPicker4(e, t) {
    ...
    const v = ((b = t.step) == null ? void 0 : b[3]) ?? 0.01, { element: T, update: y } = this.createSliderRow({
      label: "Alpha",
    ...
    return s.appendChild(o), s.appendChild(l), s.appendChild(T), {
      element: s,
      update: (m) => {
        const h = m, f = this.rgbToHex(h);
        u.value = f, d.style.backgroundColor = f, c.textContent = f, y(h[3]);
      }
    };
  }
  // ===========================================================================
  ...
  createVecSliders(e, t, n) {
    ...
    return o.forEach((u, d) => {
      var _, m, h;
      const v = ((_ = t.step) == null ? void 0 : _[d]) ?? 0.01, { element: T, update: y } = this.createSliderRow({
        label: u,
        min: ((m = t.min) == null ? void 0 : m[d]) ?? 0,
        max: ((h = t.max) == null ? void 0 : h[d]) ?? 1,
        step: v,
        value: r[d],
        format: (f) => this.formatNumber(f, v),
        onInput: (f) => {
          const p = this.values[e];
          p[d] = f, this.onChange(e, [...p]);
        }
      }), b = T.querySelector(".uniform-control-label-row");
      if (b) {
        b.classList.add("uniform-control-vec-slider-row");
        const f = b.querySelector(".uniform-control-label");
        f && f.classList.add("uniform-control-vec-component");
        const p = b.querySelector(".uniform-control-value");
        p && p.classList.add("uniform-control-vec-value");
      }
      const w = T.querySelector(".uniform-control-slider");
      w && w.classList.add("uniform-control-vec-slider"), l.push(y), a.appendChild(T);
    }), {
      element: a,
      update: (u) => {
        const d = u;
  ...
  resetToDefaults() {
    for (const [e, t] of Object.entries(this.uniforms))
      P(t) || t.hidden || (this.setValue(e, t.value), this.onChange(e, t.value));
  }
  /**
...
function xt(i, e) {
  const t = _t(i, e), n = new Blob([t], { type: "text/html" }), s = `${i.root.split("/").pop() || "shader"}.html`, o = URL.createObjectURL(n), a = document.createElement("a");
...
function _t(i, e) {
  ...
  for (const E of o) {
    ...
    if (!C)
      continue;
    const k = C.channels.map((S) => S.kind === "buffer" ? `buffer:${S.buffer}` : S.kind === "texture" ? "procedural" : S.kind === "keyboard" ? (c = !0, "keyboard") : S.kind === "script" ? (l = !0, `script:${S.name}`) : S.kind === "audio" || S.kind === "webcam" || S.kind === "video" ? "black" : "none");
    a.push({
      name: E,
      source: C.glslSource,
      channels: C.channels,
  ...
  const u = !!((p = i.script) != null && p.setup || (g = i.script) != null && g.onFrame), d = Object.entries(i.uniforms).filter(([, E]) => !P(E)), v = [];
  for (const [E, C] of d) {
    if (P(C))
      continue;
    const k = r[E] ?? C.value;
  ...
  const T = d.map(([E, C]) => `uniform ${C.type === "bool" ? "bool" : C.type} ${E};`).join(`
`), y = s.map((E) => `// Array uniform: ${E.name} (max ${E.count})
...
`), b = s.map((E) => {
    const C = Array.from(E.paddedData).map((k) => k.toFixed(6)).join(", ");
    return `  { name: '${E.name}', type: '${E.type}', count: ${E.count}, binding: ${E.bindingPoint}, data: new Float32Array([${C}]) }`;
  }).join(`,
`);
  let w = "", _ = "";
  u && ((x = i.script) != null && x.setup && (w = i.script.setup.toString()), (R = i.script) != null && R.onFrame && (_ = i.script.onFrame.toString()));
  const m = c ? `
// --- Keyboard helpers ---
const int KEY_A = 65; const int KEY_B = 66; const int KEY_C = 67; const int KEY_D = 68;
const int KEY_E = 69; const int KEY_F = 70; const int KEY_G = 71; const int KEY_H = 72;
...
class kt {
  constructor(e, t, n, r) {
    this.parentContainer = e, this.canvasWidth = t, this.canvasHeight = n, this.onStartRender = r, this.cancelRenderFn = null, this.backdrop = document.createElement("div"), this.backdrop.className = "render-dialog-backdrop", this.backdrop.addEventListener("click", (x) => {
    ...
    const T = this.createField("Format"), y = document.createElement("div");
    ...
    m.htmlFor = "render-fmt-video", m.textContent = "Video (WebM)", _.appendChild(this.formatVideo), _.appendChild(m), y.appendChild(_), y.appendChild(b), T.appendChild(y), this.estimateEl = document.createElement("div"), this.estimateEl.className = "render-estimate", this.bodyEl.appendChild(c), this.bodyEl.appendChild(d), this.bodyEl.appendChild(v), this.bodyEl.appendChild(T), this.bodyEl.appendChild(this.estimateEl), this.actionsEl = document.createElement("div"), this.actionsEl.className = "render-dialog-actions";
...
class B {
  constructor(e) {
    ...
    }, this.container = e.container, this.project = e.project, this.isMultiView = Se(e.project);
    ...
    if (this.recorder = new Ct(this.primaryView.canvas, this.container, this.project.root), this.statsPanel = new Rt(this.container), this.statsPanel.updateResolution(this.primaryView.canvas.width, this.primaryView.canvas.height), this.isMultiView) {
    ...
    if (this.project.controls && !e.skipPlaybackControls && (this.playbackControls = new St(this.container, {
    ...
    !e.skipUniformsPanel && this.project.uniforms && Object.values(this.project.uniforms).some((r) => re(r)) && (this.uniformsPanel = new wt({
  ...
  runScriptOnFrame(e, t) {
    var r;
    if (!this.scriptAPI || !((r = this.project.script) != null && r.onFrame) || this.scriptErrorCount >= B.MAX_SCRIPT_ERRORS)
      return;
    ...
    try {
      ...
      this.scriptErrorCount++, console.error(`script.js onFrame() threw (${this.scriptErrorCount}/${B.MAX_SCRIPT_ERRORS}):`, s), this.primaryView.runtimeErrorOverlay.showError("onFrame", s), this.scriptErrorCount >= B.MAX_SCRIPT_ERRORS && (console.warn("script.js onFrame() disabled after too many errors"), this.primaryView.runtimeErrorOverlay.showDisabled());
    }
  ...
  exportHTML() {
    ...
    xt(this.project, this.primaryView.engine);
  }
  openRenderDialog() {
  ...
  async renderVideoFrames(e, t, n, r) {
    ...
    for (let b = 0; b < e; b++) {
      ...
      w != null && w.requestFrame && w.requestFrame(), r(b + 1, e), b % 10 === 0 && await new Promise((_) => setTimeout(_, 0));
    }
    l.stop(), await d;
    const v = new Blob(u, { type: "video/webm" }), T = URL.createObjectURL(v), y = document.createElement("a");
    y.href = T, y.download = `render_${s.width}x${s.height}_${t}fps.webm`, y.click(), URL.revokeObjectURL(T);
  }
  stepForRender(e, t) {
    var s;
    const n = e / t, r = 1 / t;
  ...
  setupGlobalShortcuts() {
    this.globalKeyHandler = (e) => {
      B.isTextInput(e) || e.code === "KeyS" && !e.repeat && (e.preventDefault(), this.screenshot());
    }, document.addEventListener("keydown", this.globalKeyHandler);
  }
  setupKeyboardShortcuts() {
    this.controlsKeyHandler = (e) => {
      B.isTextInput(e) || (e.code === "Space" && !e.repeat && (e.preventDefault(), this.togglePlayPause()), e.code === "KeyR" && !e.repeat && (e.preventDefault(), this.reset()));
  ...
  createPanel(e) {
    ...
    `, a.addEventListener("click", () => {
      ...
      u.className = "uniforms-container", this.controls = new Ae({
        ...
        onChange: (d, v) => {
          var T, y;
          (y = (T = this.opts).onSetUniformValue) == null || y.call(T, d, v);
        }
...
class Ft {
  ...
  async buildEditorPanel() {
    try {
      const { EditorPanel: e } = await Promise.resolve().then(() => pn);
      if (this._disposed)
        return;
  ...
  buildTabBar() {
    ...
    const s = async (o) => {
      ...
      if (this.activeTabIndex = o, t.querySelectorAll(".tabbed-tab-button").forEach((c, l) => {
        ...
        try {
          const { createEditor: l } = await Promise.resolve().then(() => ze);
...
async function It(i, e) {
  ...
  return n || i.classList.add("unstyled"), oe(t.theme), Se(t) ? Ot(i, t, r) : Nt(i, t, r);
}
function Nt(i, e, t) {
  const n = Mt(e.layout, {
    container: i,
...
function Ot(i, e, t) {
  const n = e.views.map((a) => a.name), r = Ut(e.viewLayout, {
    container: i,
    project: e,
    viewNames: n
  }), s = new B({
...
const ie = ["Image", "BufferA", "BufferB", "BufferC", "BufferD"], zt = ["BufferA", "BufferB", "BufferC", "BufferD"], Fe = ["iChannel0", "iChannel1", "iChannel2", "iChannel3"], Vt = "default", Be = !0, Ht = "light", De = /* @__PURE__ */ new Set([
  "iResolution",
  "iTime",
  "iTimeDelta",
  "iFrame",
  ...
  "iPinchDelta",
  "iPinchCenter"
]), jt = /^[a-zA-Z_][a-zA-Z0-9_]*$/, Kt = /* @__PURE__ */ new Set([
  "attribute",
  "const",
  "uniform",
  "varying",
...
function O(i) {
  return jt.test(i) && !Kt.has(i);
}
const be = /* @__PURE__ */ new Set(["fullscreen", "default", "split", "tabbed"]), Q = /* @__PURE__ */ new Set(["light", "dark", "system"]), Xt = /* @__PURE__ */ new Set([
  "mode",
  "title",
  "author",
  "description",
  ...
  "views"
  // multi-view projects
]), Gt = /* @__PURE__ */ new Set(["source", "iChannel0", "iChannel1", "iChannel2", "iChannel3"]), Te = /* @__PURE__ */ new Set(["keyboard", "audio", "webcam"]);
function Yt(i, e) {
  const t = [], n = [];
  for (const s of Object.keys(i))
    Xt.has(s) || t.push(`Unknown config key '${s}'`);
  ...
  if (i.textures && typeof i.textures == "object")
    for (const [s, o] of Object.entries(i.textures))
      O(s) || n.push(`Texture name '${s}' is not a valid GLSL identifier`), r.has(s) && n.push(`Texture name '${s}' collides with a buffer name`), typeof o != "string" ? n.push(`Texture source for '${s}' must be a string`) : !Te.has(o) && !/\.\w+$/.test(o) && !O(o) && n.push(`Invalid texture source '${o}' for '${s}'. Expected a file path with extension, a script texture name, or one of: ${[...Te].join(", ")}`);
  for (const s of ie) {
    const o = i[s];
    if (!(!o || typeof o != "object")) {
      for (const a of Object.keys(o))
        Gt.has(a) || t.push(`Unknown key '${a}' in pass '${s}'`);
      for (const a of Fe) {
        const c = o[a];
...
const xe = /* @__PURE__ */ new Set(["float", "int", "bool", "vec2", "vec3", "vec4"]), _e = /* @__PURE__ */ new Set(["float", "vec2", "vec3", "vec4", "mat3", "mat4"]), Qt = {
  vec2: 2,
  vec3: 3,
  vec4: 4
};
function Wt(i, e) {
  for (const [t, n] of Object.entries(i)) {
    ...
    const s = n;
    if (!xe.has(s.type))
      throw new Error(`${r}: invalid type '${s.type}'. Expected one of: ${[...xe].join(", ")}`);
    switch (s.type) {
      case "float":
      case "int":
...
function q(i, e, t, n, r, s) {
  if (!i)
    return { kind: "none" };
  const o = Le(i);
  return o ? Zt(o, e, t, n, r, s) : { kind: "none" };
}
async function Jt(i, e, t) {
  ...
  return n ? (Yt(n, e), n.mode === "shadertoy" ? tn(i, e, n, t) : nn(i, e, n, t)) : en(i, e, t);
}
...
async function tn(i, e, t, n) {
      ...
      if (d)
        for (const v of Fe) {
          ...
          if (y && "texture" in y && !c.has(y.texture)) {
            const b = await n.textureUrlResolver(y.texture), w = y.texture.split("/").pop(), _ = a.register(b, y.filter, y.wrap, w);
            c.set(y.texture, _);
          }
        }
    }
  ...
  for (const u of ie) {
    ...
    if (!d)
      continue;
    const v = d.source ?? $t(u), T = i.joinPath(e, v);
    if (!await i.exists(T))
      throw new Error(`Source GLSL file for pass '${u}' not found at '${v}' in '${e}'.`);
    const y = await i.readText(T), b = [
      q(d.iChannel0, u, "iChannel0", e, a, c),
      q(d.iChannel1, u, "iChannel1", e, a, c),
      q(d.iChannel2, u, "iChannel2", e, a, c),
      q(d.iChannel3, u, "iChannel3", e, a, c)
    ];
    l[u] = { name: u, glslSource: y, channels: b };
  }
  if (!l.Image)
...
async function nn(i, e, t, n) {
  t.uniforms && Wt(t.uniforms, e);
...
const rn = /* @__PURE__ */ new Set(["keyboard", "audio", "webcam"]);
async function sn(i, e, t, n, r) {
  ...
  for (const [y, b] of a)
    l.set(y, { kind: "buffer", buffer: b, current: !1 });
  for (const [y, b] of Object.entries(t.textures ?? {}))
    if (b === "keyboard")
      ...
      let w;
      r != null && r.textureUrlResolver ? w = await r.textureUrlResolver(b) : w = b;
      const _ = c.register(w);
      l.set(y, { kind: "texture", name: _, cubemap: !1 });
    } else rn.has(b) || l.set(y, { kind: "script", name: b });
...
async function on(i, e, t, n, r) {
  const s = i.startsWith("./") ? i : `./${i}`, o = `${s}/config.json`;
  let a;
  if (o in t && (a = await t[o]()), a && We(a))
    return qt(a, s), an(s, a, e, n, r);
  const c = await Ie(s, r), l = Ne(e, n), d = await Jt(l, s, {
    config: a,
    script: c,
    textureUrlResolver: async (v) => {
      const T = `${s}/${v.replace(/^\.\//, "")}`;
      return l.resolveImageUrl(T);
    }
  });
  return a != null && a.title || (d.meta.title = Oe(s)), d;
}
async function an(i, e, t, n, r) {
  const s = Ne(t, n), o = await Ie(i, r);
  let a = null;
  const c = `${i}/common.glsl`;
  M(t, c) && (a = await s.readText(c));
  const l = [], u = [
    { kind: "none" },
    { kind: "none" },
    { kind: "none" },
  ...
  for (const d of e.views) {
    const v = `${i}/${d}.glsl`, T = `${i}/${d}/image.glsl`;
    let y;
    if (M(t, v))
      y = await s.readText(v);
    else if (M(t, T))
      y = await s.readText(T);
    else
...
const cn = /* @__PURE__ */ Object.assign({ "./shaders/parameter-space-quadpt/common.glsl": () => Promise.resolve().then(() => En).then((i) => i.default), "./shaders/parameter-space-quadpt/invertedStrip.glsl": () => Promise.resolve().then(() => bn).then((i) => i.default), "./shaders/parameter-space-quadpt/resulting_pack_mouse.glsl": () => Promise.resolve().then(() => wn).then((i) => i.default), "./shaders/parameter-space-quadpt/resulting_pack_pt1.glsl": () => Promise.resolve().then(() => _n).then((i) => i.default), "./shaders/parameter-space-quadpt/resulting_pack_pt2.glsl": () => Promise.resolve().then(() => Rn).then((i) => i.default), "./shaders/parameter-space-quadpt/resulting_pack_pt3.glsl": () => Promise.resolve().then(() => kn).then((i) => i.default), "./shaders/parameter-space-quadpt/resulting_pack_pt4.glsl": () => Promise.resolve().then(() => An).then((i) => i.default), "./shaders/parameter-space-quadpt/stripPack.glsl": () => Promise.resolve().then(() => Fn).then((i) => i.default) }), ln = /* @__PURE__ */ Object.assign({ "./shaders/parameter-space-quadpt/config.json": () => Promise.resolve().then(() => Dn).then((i) => i.default) }), un = /* @__PURE__ */ Object.assign({}), dn = /* @__PURE__ */ Object.assign({ "./shaders/parameter-space-quadpt/script.js": () => Promise.resolve().then(() => On) });
let ne = null;
async function hn() {
  return ne || (ne = await on("shaders/parameter-space-quadpt", cn, ln, un, dn)), ne;
}
async function $n(i, e = {}) {
  const t = await hn();
  return It(i, { project: t, ...e });
}
class mn {
  constructor(e, t) {
  ...
  async showTab(e) {
    ...
    if (this.tabBar.querySelectorAll(".editor-tab-button").forEach((n, r) => {
      ...
      try {
        const { createEditor: s } = await Promise.resolve().then(() => ze);
...
var Ce = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {}, $e = { exports: {} };
(function(i) {
  ...
  var t = function(n) {
    var r = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, s = 0, o = {}, a = {
        ...
        insertBefore: function(m, h, f, p) {
          ...
          for (var R in g)
            if (g.hasOwnProperty(R)) {
              ...
              f.hasOwnProperty(R) || (x[R] = g[R]);
            }
          var C = p[m];
          return p[m] = x, a.languages.DFS(a.languages, function(k, S) {
            S === C && k != m && (this[k] = x);
          }), x;
        },
        ...
        DFS: function m(h, f, p, g) {
          ...
          for (var R in h)
            if (h.hasOwnProperty(R)) {
              ...
              C === "Object" && !g[x(E)] ? (g[x(E)] = !0, m(E, f, null, g)) : C === "Array" && !g[x(E)] && (g[x(E)] = !0, m(E, f, R, g));
            }
        }
      },
      plugins: {},
      ...
      highlightElement: function(m, h, f) {
        ...
        function C(S) {
          E.highlightedCode = S, a.hooks.run("before-insert", E), E.element.innerHTML = E.highlightedCode, a.hooks.run("after-highlight", E), a.hooks.run("complete", E), f && f.call(E.element);
        }
        if (a.hooks.run("before-sanity-check", E), x = E.element.parentElement, x && x.nodeName.toLowerCase() === "pre" && !x.hasAttribute("tabindex") && x.setAttribute("tabindex", "0"), !E.code) {
          a.hooks.run("complete", E), f && f.call(E.element);
          return;
        }
        ...
        if (h && n.Worker) {
          var k = new Worker(a.filename);
          k.onmessage = function(S) {
            C(S.data);
      ...
      tokenize: function(m, h) {
        ...
        var g = new d();
        return v(g, g.head, m), u(m, g, h, g.head, 0), y(g);
      },
      /**
       * @namespace
       * @memberof Prism
      ...
      for (var R in f)
        if (!(!f.hasOwnProperty(R) || !f[R])) {
          ...
          for (var C = 0; C < E.length; ++C) {
            if (x && x.cause == R + "," + C)
              return;
            var k = E[C], S = k.inside, U = !!k.lookbehind, ce = !!k.greedy, Ge = k.alias;
            if (ce && !k.pattern.global) {
              var Ye = k.pattern.toString().match(/[imsuy]*$/)[0];
              k.pattern = RegExp(k.pattern.source, Ye + "g");
            }
            for (var le = k.pattern || k, A = p.next, F = g; A !== h.tail && !(x && F >= x.reach); F += A.value.length, A = A.next) {
              var I = A.value;
              ...
              if (!(I instanceof c)) {
                var H = 1, L;
                if (ce) {
                  if (L = l(le, F, m, U), !L || L.index >= m.length)
                    break;
                  var j = L.index, qe = L.index + L[0].length, D = F;
                  for (D += A.value.length; j >= D; )
                    A = A.next, D += A.value.length;
                  if (D -= A.value.length, F = D, A.value instanceof c)
                    continue;
                  for (var N = A; N !== h.tail && (D < qe || typeof N.value == "string"); N = N.next)
                    H++, D += N.value.length;
                  H--, I = m.slice(F, D), L.index -= F;
                } else if (L = l(le, 0, I, U), !L)
                  continue;
                var j = L.index, K = L[0], Z = I.slice(0, j), ue = I.slice(j + K.length), J = F + I.length;
                x && J > x.reach && (x.reach = J);
                var X = A.prev;
                Z && (X = v(h, X, Z), F += Z.length), T(h, X, H);
                var Qe = new c(R, S ? a.tokenize(K, S) : K, Ge, K);
                if (A = v(h, X, Qe), ue && v(h, A, ue), H > 1) {
                  var ee = {
                    cause: R + "," + C,
                    reach: J
                  };
                  u(m, h, f, A.prev, F, ee), x && ee.reach > x.reach && (x.reach = ee.reach);
                }
              }
            }
          }
    ...
    if (!a.manual) {
      ...
      _ === "loading" || _ === "interactive" && b && b.defer ? document.addEventListener("DOMContentLoaded", w) : window.requestAnimationFrame ? window.requestAnimationFrame(w) : window.setTimeout(w, 16);
    }
    return a;
  }(e);
  i.exports && (i.exports = t), typeof Ce < "u" && (Ce.Prism = t), t.languages.markup = {
    ...
    function v(b, w, _) {
      ...
      m.open("GET", b, !0), m.onreadystatechange = function() {
        m.readyState == 4 && (m.status < 400 && m.responseText ? w(m.responseText) : m.status >= 400 ? _(r(m.status, m.statusText)) : _(s));
      }, m.send(null);
    }
    function T(b) {
    ...
    t.hooks.add("before-highlightall", function(b) {
      ...
      if (w.matches(d)) {
        ...
        _.textContent = n;
        var m = w.getAttribute("data-src"), h = b.language;
        if (h === "none") {
        ...
        p && p.loadLanguages(h), v(
          m,
          function(g) {
            w.setAttribute(a, l);
            var x = T(w.getAttribute("data-range"));
            if (x) {
              var R = g.split(/\r\n?|\n/g), E = x[0], C = x[1] == null ? R.length : x[1];
              E < 0 && (E += R.length), E = Math.max(0, Math.min(E - 1, R.length)), C < 0 && (C += R.length), C = Math.max(0, Math.min(C, R.length)), g = R.slice(E, C).join(`
`), w.hasAttribute("data-start") || w.setAttribute("data-start", String(E + 1));
            }
...
function gn(i, e, t) {
  ...
  function l() {
    const v = o.value;
    c.textContent = v + `
`, fn.highlightElement(c);
    const T = v.split(`
`);
    r.innerHTML = T.map((y, b) => `<span>${b + 1}</span>`).join(""), t && t(v);
  }
  function u() {
  ...
  function d(v) {
    if (v.key === "Tab") {
      v.preventDefault();
      const T = o.selectionStart, y = o.selectionEnd, b = o.value;
      o.value = b.substring(0, T) + "  " + b.substring(y), o.selectionStart = o.selectionEnd = T + 2, l();
    }
  }
...
const ze = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  createEditor: gn
}, Symbol.toStringTag, { value: "Module" })), vn = `float EPS=1e-4;
...
`, En = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: vn
}, Symbol.toStringTag, { value: "Module" })), yn = `// --- 4. RENDER LOGIC ---
...
//     fragColor = vec4(color, 1.0);`, _n = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: xn
...
`, An = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Pn
}, Symbol.toStringTag, { value: "Module" })), Ln = `// --- 4. RENDER LOGIC ---
vec2 normalize_coord(vec2 fragCoord) {
    vec2 uv = fragCoord / iResolution.xy;
    uv = uv - vec2(0.5, 0.5);
    uv.x *= iResolution.x / iResolution.y;
...
`, Fn = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Ln
}, Symbol.toStringTag, { value: "Module" })), Ve = "ACP parameter space with packing equivalent points", He = "Visualize the parameter space of Descartes Quadruples, highlighting points that correspond to the same packing configuration.", je = [
  "stripPack",
  "invertedStrip"
], Ke = "split", Xe = {
  u_zoom_stripPack: {
  ...
  stripPack: { zoom: 1, panX: 0, panY: 0, zoomKey: "u_zoom_stripPack", panKey: "u_pan_stripPack" }
  //resulting_pack: { zoom: 1.0, panX: 0.0, panY: 0.0, zoomKey: 'u_zoom_resulting_pack', panKey: 'u_pan_resulting_pack' },
  //invertedStrip:  { zoom: 1.0, panX: 0.0, panY: 0.0, zoomKey: 'u_zoom_invertedStrip',  panKey: 'u_pan_invertedStrip' },
};
function Re(i, e, t, n, r) {
  const s = i.querySelector("canvas"), o = s ? s.clientWidth / s.clientHeight : 1, a = 6.5 * 0.05 / e.zoom;
  e.panX += n * a * o, e.panY += r * a, t.setUniformValue(e.panKey, [e.panX, e.panY]);
}
function Un(i, e, t) {
  const n = document.createElement("div"), r = 96, s = 12;
  function o() {
    const l = i.getBoundingClientRect();
    n.style.left = `${l.right - s - r}px`, n.style.top = `${l.bottom - s - r}px`;
  }
  n.style.cssText = `
    position: fixed;
    display: grid;
  ...
  for (const [l, u, d, v, T] of c) {
    const y = document.createElement("button");
    if (y.textContent = d, y.style.cssText = `grid-column: ${v}; grid-row: ${T};` + a, d === "↺")
      y.addEventListener("mousedown", (w) => {
        w.stopPropagation(), e.zoom = 1, e.panX = 0, e.panY = 0, t.setUniformValue(e.zoomKey, e.zoom), t.setUniformValue(e.panKey, [e.panX, e.panY]);
      });
    else {
      let w = null;
      y.addEventListener("mousedown", (_) => {
        _.stopPropagation(), Re(i, e, t, l, u), w = setInterval(() => Re(i, e, t, l, u), 80);
...
function In(i, e, t) {
  const n = document.querySelector(`[data-view-name="${e}"]`);
  n && (n.style.position = "relative", Un(n, t, i), n.addEventListener("wheel", (r) => {
    const s = n.querySelector("canvas"), o = r.deltaY > 0 ? 1 / 1.15 : 1.15, a = t.zoom;
    if (t.zoom *= o, t.zoom = Math.max(1e-6, Math.min(t.zoom, 1e3)), s) {
      const c = s.getBoundingClientRect(), l = c.width / c.height, u = (r.clientX - c.left) / c.width, d = 1 - (r.clientY - c.top) / c.height, v = (u - 0.5) * l * 6.5, T = (d - 0.5) * 6.5;
      t.panX += v * (1 / a - 1 / t.zoom), t.panY += T * (1 / a - 1 / t.zoom);
    }
    i.setUniformValue(t.zoomKey, t.zoom), i.setUniformValue(t.panKey, [t.panX, t.panY]), r.preventDefault();
  }, { passive: !1 }));
}
function Nn(i) {
