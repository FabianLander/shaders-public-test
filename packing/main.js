...
function ge(i, e, t) {
  const n = fe(i, i.VERTEX_SHADER, e), r = fe(i, i.FRAGMENT_SHADER, t), s = i.createProgram();
  if (!s)
...
function ve(i, e) {
  ...
  if (n !== i.FRAMEBUFFER_COMPLETE)
    throw i.deleteFramebuffer(t), new Error(`Framebuffer incomplete: ${ct(i, n)}`);
...
`, mt = `// --- Keyboard helpers (auto-injected) ---
// Letter keys
...
`), t.viewNames && t.viewNames.length > 1) {
    ...
    n.push(""), t.namedSamplers.has("keyboard") && (n.push(mt), n.push(""));
...
`), t.viewNames && t.viewNames.length > 1) {
  ...
  if (r.length > 0) {
    ...
    for (const [d, g] of r) {
      const y = g.type === "bool" ? "bool" : g.type;
      n.push(`uniform ${y} ${d};`);
    }
    n.push("");
  }
...
class gt {
  ...
  updateAudioTexture(e) {
    var t;
    (t = this._audioTexture) != null && t.analyser && (this._audioTexture.analyser.getByteFrequencyData(this._audioTexture.frequencyData), this._audioTexture.analyser.getByteTimeDomainData(this._audioTexture.waveformData), st(e, this._audioTexture.texture, this._audioTexture.frequencyData, this._audioTexture.waveformData));
  }
  updateVideoTextures(e) {
    for (const t of this._videoTextures)
      !t.ready || !t.video || t.video.readyState < HTMLMediaElement.HAVE_CURRENT_DATA || (ot(e, t.texture, t.video), t.video.videoWidth > 0 && (t.width = t.video.videoWidth, t.height = t.video.videoHeight));
  }
  // ===========================================================================
  // Cleanup
  // ===========================================================================
...
class vt {
  ...
  initializeDefaults() {
    for (const [e, t] of Object.entries(this.definitions))
      A(t) ? this.values[e] = new Float32Array(H(t.type, t.count)) : this.values[e] = this.cloneValue(t.value);
  }
  /**
  ...
  reset(e) {
    ...
    return t ? (A(t) ? this.values[e] = new Float32Array(H(t.type, t.count)) : this.values[e] = this.cloneValue(t.value), !0) : !1;
  }
  /**
  ...
  getDefault(e) {
    const t = this.definitions[e];
    if (t)
      return A(t) ? new Float32Array(H(t.type, t.count)) : this.cloneValue(t.value);
  }
  /**
...
class Et {
  constructor(e, t) {
    this._ubos = [], this._dirtyScalars = /* @__PURE__ */ new Set(), this._uniforms = t, this._store = new vt(t), this.initUBOs(e);
  ...
  set(e, t) {
    ...
    if (this._store.set(e, t), A(n)) {
      ...
      if (r) {
        const s = t, o = H(n.type, n.count), a = H(n.type, 1);
        if (s.length > o) {
        ...
        const c = s.length / a, l = dt(n.type, c, s, r.paddedData);
        l !== r.paddedData && r.paddedData.set(l);
        const u = ut(n.type, c), d = r.paddedData.length;
...
class Te {
  ...
  updateTexture(e, t, n, r) {
    ...
    if (s && s.width === t && s.height === n && s.isFloat === o) {
      ...
      const a = at(this.gl, (s == null ? void 0 : s.texture) ?? null, t, n, r);
  ...
  step(e, t, n, r, s) {
    ...
    for (const g of d) {
      const y = this._passes.find((T) => T.name === g);
  ...
  resize(e, t) {
    ...
    for (const r of this._passes)
      n.deleteTexture(r.currentTexture), n.deleteTexture(r.previousTexture), n.deleteFramebuffer(r.framebuffer), r.currentTexture = q(n, e, t), r.previousTexture = q(n, e, t), r.framebuffer = ve(n, r.currentTexture);
  }
  /**
  ...
  updateKeyboardTexture() {
    this._keyboardTexture && it(this.gl, this._keyboardTexture.texture, this._keyStates, this._toggleStates);
  }
  /**
   ...
   */
  initRuntimePasses() {
    const e = this.gl, t = this.project, n = et(e);
    this._sharedVAO = n;
    ...
    for (const s of r) {
      ...
      try {
        const l = ge(e, be, a), u = this.cacheUniformLocations(l, o.namedSamplers), d = q(e, this._width, this._height), g = q(e, this._width, this._height), y = ve(e, d), T = {
          name: s,
        ...
        if (d) {
          ...
          if (c.commonStartLine > 0 && c.commonLines > 0) {
            const E = c.commonStartLine + c.commonLines - 1;
            T >= c.commonStartLine && T <= E && (g = !0, y = T - c.commonStartLine + 1);
          }
          !g && c.userCodeStartLine > 0 && T >= c.userCodeStartLine && (y = T - c.userCodeStartLine + 1);
        }
...
class bt {
  ...
  show(e, t) {
    ...
    const n = e.filter((u) => u.isFromCommon), r = e.filter((u) => !u.isFromCommon), c = [...n.length > 0 ? [n[0]] : [], ...r].map(({ passName: u, error: d, isFromCommon: g, originalLine: y, lineMapping: T }) => {
      const E = d.replace(`Shader compilation failed:
`, ""), x = y;
      let _ = E;
      x !== null && (_ = E.replace(/ERROR:\s*\d+:(\d+):/g, `ERROR: 0:${x}:`));
      let m = null;
      if (g)
        m = t.commonSource;
      else {
      ...
      return {
        passName: g ? "common.glsl" : u,
        error: Tt(_, T, g),
        codeContext: x !== null && m ? xt(m, x) : null
      };
    }).map(({ passName: u, error: d, codeContext: g }) => `
      <div class="error-section">
        <div class="error-pass-name">${u}</div>
        <pre class="error-content">${De(d)}</pre>
...
`).map((n) => {
    ...
    if (r) {
      ...
      let c = a;
      return t && e.commonStartLine > 0 ? c = a - e.commonStartLine + 1 : e.userCodeStartLine > 0 && a >= e.userCodeStartLine && (c = a - e.userCodeStartLine + 1), `Line ${c}: ${yt(o)}`;
    }
    return n;
  }).join(`
`);
...
`);
  ...
  return t.slice(r, s).map((a, c) => {
    const l = r + c + 1, u = l === e, d = String(l).padStart(4, " "), g = De(a);
    return u ? `<span class="error-line-highlight">${d} │ ${g}</span>` : `<span class="context-line">${d} │ ${g}</span>`;
  }).join("");
}
function De(i) {
...
`) : "";
    this.overlay.innerHTML = `
      <div class="script-error-content">
        ...
        ${r ? `<pre class="script-error-stack">${Z(r)}</pre>` : ""}
      </div>
    `, this.wireClose(), this.autoHideTimer = setTimeout(() => this.hide(), j.AUTO_HIDE_MS);
  }
  /**
...
const X = {};
for (let i = 0; i < 26; i++)
  X[`Key${String.fromCharCode(65 + i)}`] = 65 + i;
for (let i = 0; i < 10; i++)
  X[`Digit${i}`] = 48 + i;
for (let i = 1; i <= 12; i++)
  X[`F${i}`] = 111 + i;
...
class xe {
  ...
  constructor(e) {
    ...
    this.gl = t, this.setupContextLossHandling(), this.updateCanvasSize(), this._engine = new Te({
      gl: this.gl,
      project: e.project,
    ...
    }), this._resizeObserver.observe(this.container), this.input = new wt(this.canvas, this._pixelRatio), this.input.onFirstGesture = () => this.initMediaOnGesture(), this.initVideoFiles();
  }
  // ===========================================================================
  // Per-Frame Rendering
  // ===========================================================================
  ...
  handleContextRestored() {
    ...
    try {
      this._engine.dispose(), this._engine = new Te({
        gl: this.gl,
...
class Me {
  ...
  createVec2Pad(e, t) {
    ...
    g.className = "uniform-control-xy-handle", d.appendChild(g);
    const y = (p) => {
      const v = (p[0] - r[0]) / (s[0] - r[0]) * 100, w = (1 - (p[1] - r[1]) / (s[1] - r[1])) * 100;
      g.style.left = `${v}%`, g.style.top = `${w}%`;
    };
    y(n);
    let T = !1;
    const E = (p) => {
      const v = d.getBoundingClientRect(), w = "touches" in p ? p.touches[0].clientX : p.clientX, R = "touches" in p ? p.touches[0].clientY : p.clientY;
      let b = Math.max(0, Math.min(1, (w - v.left) / v.width)), C = Math.max(0, Math.min(1, (R - v.top) / v.height));
      const F = r[0] + b * (s[0] - r[0]), S = r[1] + (1 - C) * (s[1] - r[1]), I = [F, S];
      this.values[e] = I, g.style.left = `${b * 100}%`, g.style.top = `${C * 100}%`, u.textContent = this.formatVec2(I), this.onChange(e, I);
    }, x = (p) => {
      T = !0, E(p), p.preventDefault();
    }, _ = (p) => {
      T && E(p);
    }, m = () => {
      T = !1;
    ...
    return d.addEventListener("touchstart", h), document.addEventListener("touchmove", f), document.addEventListener("touchend", m), this.documentListeners.push({ type: "touchmove", handler: f }), this.documentListeners.push({ type: "touchend", handler: m }), a.appendChild(c), a.appendChild(d), {
      ...
      update: (p) => {
        const v = p;
        y(v), u.textContent = this.formatVec2(v);
      }
    };
  }
  // ===========================================================================
  ...
  createColorPicker(e, t) {
    ...
    return d.className = "uniform-control-color-swatch", d.style.backgroundColor = this.rgbToHex(n), u.addEventListener("input", () => {
      const g = this.hexToRgb(u.value);
      this.values[e] = g, c.textContent = u.value, d.style.backgroundColor = u.value, this.onChange(e, g);
    }), d.addEventListener("click", () => u.click()), l.appendChild(d), l.appendChild(u), s.appendChild(o), s.appendChild(l), {
      element: s,
      update: (g) => {
        const y = this.rgbToHex(g);
  ...
  createColorPicker4(e, t) {
    ...
    const g = ((E = t.step) == null ? void 0 : E[3]) ?? 0.01, { element: y, update: T } = this.createSliderRow({
      label: "Alpha",
      min: ((x = t.min) == null ? void 0 : x[3]) ?? 0,
      max: ((_ = t.max) == null ? void 0 : _[3]) ?? 1,
      step: g,
      value: n[3],
      format: (m) => this.formatNumber(m, g),
      onInput: (m) => {
    ...
    return s.appendChild(o), s.appendChild(l), s.appendChild(y), {
      ...
      update: (m) => {
        ...
        u.value = f, d.style.backgroundColor = f, c.textContent = f, T(h[3]);
      }
    };
  }
  // ===========================================================================
  ...
  createVecSliders(e, t, n) {
    ...
    return o.forEach((u, d) => {
      var _, m, h;
      const g = ((_ = t.step) == null ? void 0 : _[d]) ?? 0.01, { element: y, update: T } = this.createSliderRow({
        label: u,
        min: ((m = t.min) == null ? void 0 : m[d]) ?? 0,
        max: ((h = t.max) == null ? void 0 : h[d]) ?? 1,
        step: g,
        value: r[d],
        format: (f) => this.formatNumber(f, g),
        onInput: (f) => {
          const p = this.values[e];
          p[d] = f, this.onChange(e, [...p]);
        }
      }), E = y.querySelector(".uniform-control-label-row");
      if (E) {
        E.classList.add("uniform-control-vec-slider-row");
        const f = E.querySelector(".uniform-control-label");
        f && f.classList.add("uniform-control-vec-component");
        const p = E.querySelector(".uniform-control-value");
...
class _t {
  constructor(e) {
    ...
    `, this.toggleButton.addEventListener("click", () => this.toggle()), this.wrapper.appendChild(this.toggleButton), this.panel = document.createElement("div"), this.panel.className = "uniforms-panel", !Object.values(e.uniforms).some((a) => ce(a))) {
    ...
    const o = document.createElement("div");
    o.className = "uniforms-panel-content", this.panel.appendChild(o), this.controls = new Me({
      container: o,
      uniforms: e.uniforms,
...
const re = (i) => i.replace(/\\/g, "\\\\").replace(/`/g, "\\`").replace(/\$/g, "\\$");
function Ct(i, e) {
  const t = Rt(i, e), n = new Blob([t], { type: "text/html" }), s = `${i.root.split("/").pop() || "shader"}.html`, o = URL.createObjectURL(n), a = document.createElement("a");
...
function Rt(i, e) {
  ...
  const u = !!((p = i.script) != null && p.setup || (v = i.script) != null && v.onFrame), d = Object.entries(i.uniforms).filter(([, b]) => !A(b)), g = [];
  for (const [b, C] of d) {
    if (A(C))
      continue;
    const F = r[b] ?? C.value;
    if (C.type === "float" || C.type === "int")
      ...
      const S = F;
      g.push(`  '${b}': [${S[0]}, ${S[1]}, ${S[2]}]`);
    } else if (C.type === "vec4") {
      const S = F;
      g.push(`  '${b}': [${S[0]}, ${S[1]}, ${S[2]}, ${S[3]}]`);
    }
  }
  const y = d.map(([b, C]) => `uniform ${C.type === "bool" ? "bool" : C.type} ${b};`).join(`
`), T = s.map((b) => `// Array uniform: ${b.name} (max ${b.count})
...
`), E = s.map((b) => {
    const C = Array.from(b.paddedData).map((F) => F.toFixed(6)).join(", ");
    return `  { name: '${b.name}', type: '${b.type}', count: ${b.count}, binding: ${b.bindingPoint}, data: new Float32Array([${C}]) }`;
  }).join(`,
`);
  let x = "", _ = "";
...
bool isKeyDown(int key) { return keyDown(key) > 0.5; }
bool isKeyToggled(int key) { return keyToggle(key) > 0.5; }
` : "", h = a.map((b) => `  { name: '${b.name}', source: \`${re(b.source)}\`, channels: ${JSON.stringify(b.channelTypes)} }`).join(`,
`), f = `#version 300 es
precision highp float;
...
class kt {
  constructor(e, t, n, r) {
    ...
    const g = this.createField("Duration (seconds)");
    this.durationInput = this.createNumberInput(10, 1, 3600), g.appendChild(this.durationInput);
    ...
    const E = document.createElement("div");
    E.className = "render-format-option";
    const x = document.createElement("label");
    x.htmlFor = "render-fmt-frames", x.textContent = "PNG Frames", E.appendChild(this.formatFrames), E.appendChild(x);
    const _ = document.createElement("div");
    _.className = "render-format-option";
    const m = document.createElement("label");
    m.htmlFor = "render-fmt-video", m.textContent = "Video (WebM)", _.appendChild(this.formatVideo), _.appendChild(m), T.appendChild(_), T.appendChild(E), y.appendChild(T), this.estimateEl = document.createElement("div"), this.estimateEl.className = "render-estimate", this.bodyEl.appendChild(c), this.bodyEl.appendChild(d), this.bodyEl.appendChild(g), this.bodyEl.appendChild(y), this.bodyEl.appendChild(this.estimateEl), this.actionsEl = document.createElement("div"), this.actionsEl.className = "render-dialog-actions";
    ...
    p.className = "render-btn render-btn-cancel", p.textContent = "Cancel Render", p.style.marginTop = "4px", p.addEventListener("click", () => this.cancelRender()), this.progressEl.appendChild(p), s.appendChild(o), s.appendChild(this.bodyEl), s.appendChild(this.actionsEl), s.appendChild(this.progressEl), this.backdrop.appendChild(s);
...
class M {
  constructor(e) {
    ...
    }, this.container = e.container, this.project = e.project, this.isMultiView = Le(e.project);
    ...
    if (this.isMultiView) {
      ...
      for (const o of r.views) {
        ...
        const c = this.createViewProject(r, o), l = new xe({
          container: a,
          project: c,
          pixelRatio: t,
          viewNames: s
    ...
    if (this.recorder = new St(this.primaryView.canvas, this.container, this.project.root), this.statsPanel = new Ft(this.container), this.statsPanel.updateResolution(this.primaryView.canvas.width, this.primaryView.canvas.height), this.isMultiView) {
    ...
    if (this.project.controls && !e.skipPlaybackControls && (this.playbackControls = new At(this.container, {
    ...
    !e.skipUniformsPanel && this.project.uniforms && Object.values(this.project.uniforms).some((r) => ce(r)) && (this.uniformsPanel = new _t({
  ...
  runScriptOnFrame(e, t) {
    var r;
    if (!this.scriptAPI || !((r = this.project.script) != null && r.onFrame) || this.scriptErrorCount >= M.MAX_SCRIPT_ERRORS)
      return;
    ...
    try {
      ...
      this.scriptErrorCount++, console.error(`script.js onFrame() threw (${this.scriptErrorCount}/${M.MAX_SCRIPT_ERRORS}):`, s), this.primaryView.runtimeErrorOverlay.showError("onFrame", s), this.scriptErrorCount >= M.MAX_SCRIPT_ERRORS && (console.warn("script.js onFrame() disabled after too many errors"), this.primaryView.runtimeErrorOverlay.showDisabled());
    }
  ...
  exportHTML() {
    ...
    Ct(this.project, this.primaryView.engine);
  }
  openRenderDialog() {
    new kt(this.container, this.primaryView.canvas.width, this.primaryView.canvas.height, (t) => this.renderOffline(t)).open();
  }
  renderOffline(e) {
    let t = !1;
    const n = () => {
    ...
    return (async () => {
      ...
      try {
        ...
        const g = Math.ceil(e.fps * e.duration);
        e.format === "video" ? await this.renderVideoFrames(g, e.fps, () => t, e.onProgress) : await this.renderPngFrames(g, e.fps, () => t, e.onProgress), t || e.onComplete();
      } catch (g) {
        t || e.onError(g instanceof Error ? g : new Error(String(g)));
      } finally {
  ...
  async renderVideoFrames(e, t, n, r) {
    ...
    for (let E = 0; E < e; E++) {
      ...
      this.stepForRender(E, t), this.primaryView.presentToScreen(), a.drawImage(s, 0, 0);
      const x = c.getVideoTracks()[0];
      x != null && x.requestFrame && x.requestFrame(), r(E + 1, e), E % 10 === 0 && await new Promise((_) => setTimeout(_, 0));
    }
    l.stop(), await d;
    const g = new Blob(u, { type: "video/webm" }), y = URL.createObjectURL(g), T = document.createElement("a");
...
class Lt {
  ...
  createPanel(e) {
    ...
    `, a.addEventListener("click", () => {
      this.opts.onReset();
    }), s.appendChild(a), this.panel.appendChild(s), e && Object.values(e).some((c) => ce(c))) {
      const c = document.createElement("div");
      c.className = "controls-section uniforms-section";
      const l = document.createElement("div");
      l.className = "section-label", l.textContent = "Uniforms", c.appendChild(l);
      const u = document.createElement("div");
      u.className = "uniforms-container", this.controls = new Me({
        container: u,
        uniforms: e,
        onChange: (d, g) => {
          var y, T;
          (T = (y = this.opts).onSetUniformValue) == null || T.call(y, d, g);
        }
...
class Mt {
  ...
  async buildEditorPanel() {
    try {
      const { EditorPanel: e } = await Promise.resolve().then(() => gn);
      if (this._disposed)
        return;
  ...
  buildTabBar() {
    ...
    const s = async (o) => {
      ...
      if (this.activeTabIndex = o, t.querySelectorAll(".tabbed-tab-button").forEach((c, l) => {
        ...
        try {
          const { createEditor: l } = await Promise.resolve().then(() => Xe);
...
async function Ot(i, e) {
  ...
  return n || i.classList.add("unstyled"), ue(t.theme), Le(t) ? zt(i, t, r) : $t(i, t, r);
}
function $t(i, e, t) {
  const n = It(e.layout, {
    container: i,
...
function zt(i, e, t) {
  const n = e.views.map((a) => a.name), r = Nt(e.viewLayout, {
    container: i,
    project: e,
    viewNames: n
  }), s = new M({
...
function Ue(i) {
  return typeof i == "string" ? de(i) ? { buffer: i } : i === "keyboard" ? { keyboard: !0 } : i === "audio" ? { audio: !0 } : i === "webcam" ? { webcam: !0 } : { texture: i } : i;
}
const oe = ["Image", "BufferA", "BufferB", "BufferC", "BufferD"], Ht = ["BufferA", "BufferB", "BufferC", "BufferD"], Be = ["iChannel0", "iChannel1", "iChannel2", "iChannel3"], jt = "default", Ie = !0, Xt = "light", Ne = /* @__PURE__ */ new Set([
  "iResolution",
  "iTime",
  "iTimeDelta",
  "iFrame",
  ...
  "iPinchDelta",
  "iPinchCenter"
]), Kt = /^[a-zA-Z_][a-zA-Z0-9_]*$/, Gt = /* @__PURE__ */ new Set([
  "attribute",
  "const",
  "uniform",
  "varying",
...
function V(i) {
  return Kt.test(i) && !Gt.has(i);
}
const _e = /* @__PURE__ */ new Set(["fullscreen", "default", "split", "tabbed"]), Q = /* @__PURE__ */ new Set(["light", "dark", "system"]), Yt = /* @__PURE__ */ new Set([
  "mode",
  "title",
  "author",
  "description",
  ...
  "views"
  // multi-view projects
]), Wt = /* @__PURE__ */ new Set(["source", "iChannel0", "iChannel1", "iChannel2", "iChannel3"]), Ce = /* @__PURE__ */ new Set(["keyboard", "audio", "webcam"]);
function qt(i, e) {
  const t = [], n = [];
  for (const s of Object.keys(i))
    Yt.has(s) || t.push(`Unknown config key '${s}'`);
  if (i.layout !== void 0 && !_e.has(i.layout) && n.push(`Invalid layout '${i.layout}'. Expected one of: ${[..._e].join(", ")}`), i.theme !== void 0 && !Q.has(i.theme) && n.push(`Invalid theme '${i.theme}'. Expected one of: ${[...Q].join(", ")}`), i.uniforms && typeof i.uniforms == "object")
    for (const s of Object.keys(i.uniforms))
      Ne.has(s) && n.push(`Uniform name '${s}' is reserved (built-in uniform)`), V(s) || n.push(`Uniform name '${s}' is not a valid GLSL identifier`);
  ...
  if (i.buffers) {
    ...
    for (const o of s) {
      ...
      V(o) || n.push(`Buffer name '${o}' is not a valid GLSL identifier`), r.add(o);
    }
  }
  if (i.textures && typeof i.textures == "object")
    for (const [s, o] of Object.entries(i.textures))
      V(s) || n.push(`Texture name '${s}' is not a valid GLSL identifier`), r.has(s) && n.push(`Texture name '${s}' collides with a buffer name`), typeof o != "string" ? n.push(`Texture source for '${s}' must be a string`) : !Ce.has(o) && !/\.\w+$/.test(o) && !V(o) && n.push(`Invalid texture source '${o}' for '${s}'. Expected a file path with extension, a script texture name, or one of: ${[...Ce].join(", ")}`);
  for (const s of oe) {
    const o = i[s];
    if (!(!o || typeof o != "object")) {
      for (const a of Object.keys(o))
        Wt.has(a) || t.push(`Unknown key '${a}' in pass '${s}'`);
      for (const a of Be) {
        const c = o[a];
        c && typeof c == "string" && de(c) && c !== "Image" && !i[c] && t.push(`${s}.${a} references '${c}' but no ${c} pass is configured`);
      }
    }
  }
  for (const s of t)
...
const Re = /* @__PURE__ */ new Set(["split", "quad", "grid", "inset"]);
function Zt(i, e) {
  const t = [];
  ...
  if (i.layout !== void 0 && !Re.has(i.layout) && t.push(`Invalid multi-view layout '${i.layout}'. Expected one of: ${[...Re].join(", ")}`), i.theme !== void 0 && !Q.has(i.theme) && t.push(`Invalid theme '${i.theme}'. Expected one of: ${[...Q].join(", ")}`), i.uniforms && typeof i.uniforms == "object")
    for (const n of Object.keys(i.uniforms))
      Ne.has(n) && t.push(`Uniform name '${n}' is reserved (built-in uniform)`), V(n) || t.push(`Uniform name '${n}' is not a valid GLSL identifier`);
  if (t.length > 0)
...
const Se = /* @__PURE__ */ new Set(["float", "int", "bool", "vec2", "vec3", "vec4"]), Fe = /* @__PURE__ */ new Set(["float", "vec2", "vec3", "vec4", "mat3", "mat4"]), Jt = {
  vec2: 2,
  vec3: 3,
  vec4: 4
};
function Qt(i, e) {
  for (const [t, n] of Object.entries(i)) {
    ...
    if (A(n)) {
      if (!Fe.has(n.type))
        throw new Error(`${r}: invalid array type '${n.type}'. Expected one of: ${[...Fe].join(", ")}`);
    ...
    const s = n;
    if (!Se.has(s.type))
      throw new Error(`${r}: invalid type '${s.type}'. Expected one of: ${[...Se].join(", ")}`);
    switch (s.type) {
      case "float":
      case "int":
...
function J(i, e, t, n, r, s) {
  if (!i)
    return { kind: "none" };
  const o = Ue(i);
  return o ? en(o, e, t, n, r, s) : { kind: "none" };
}
async function tn(i, e, t) {
  ...
  return n ? (qt(n, e), n.mode === "shadertoy" ? rn(i, e, n, t) : sn(i, e, n, t)) : nn(i, e, t);
}
...
async function rn(i, e, t, n) {
  ...
  const o = await Oe(i, e, t.common), a = new $e(), c = /* @__PURE__ */ new Map();
      ...
      if (d)
        for (const g of Be) {
          ...
          if (T && "texture" in T && !c.has(T.texture)) {
            const E = await n.textureUrlResolver(T.texture), x = T.texture.split("/").pop(), _ = a.register(E, T.filter, T.wrap, x);
            c.set(T.texture, _);
          }
        }
    }
  ...
  for (const u of oe) {
    ...
    if (!d)
      continue;
    const g = d.source ?? Vt(u), y = i.joinPath(e, g);
    if (!await i.exists(y))
      throw new Error(`Source GLSL file for pass '${u}' not found at '${g}' in '${e}'.`);
    const T = await i.readText(y), E = [
      J(d.iChannel0, u, "iChannel0", e, a, c),
      J(d.iChannel1, u, "iChannel1", e, a, c),
      J(d.iChannel2, u, "iChannel2", e, a, c),
      J(d.iChannel3, u, "iChannel3", e, a, c)
    ];
    l[u] = { name: u, glslSource: T, channels: E };
  }
  if (!l.Image)
...
async function sn(i, e, t, n) {
  t.uniforms && Qt(t.uniforms, e);
  const r = await Oe(i, e, t.common), s = t.buffers ?? {};
...
const on = /* @__PURE__ */ new Set(["keyboard", "audio", "webcam"]);
async function an(i, e, t, n, r) {
  ...
  for (let T = 0; T < o.length; T++)
    a.set(o[T], Ht[T]);
  const c = new $e(), l = /* @__PURE__ */ new Map();
  for (const [T, E] of a)
    l.set(T, { kind: "buffer", buffer: E, current: !1 });
  for (const [T, E] of Object.entries(t.textures ?? {}))
    if (E === "keyboard")
      ...
      let x;
      r != null && r.textureUrlResolver ? x = await r.textureUrlResolver(E) : x = E;
      const _ = c.register(x);
      l.set(T, { kind: "texture", name: _, cubemap: !1 });
    } else on.has(E) || l.set(T, { kind: "script", name: E });
...
async function cn(i, e, t, n, r) {
  ...
  let a;
  if (o in t && (a = await t[o]()), a && Qe(a))
    return Zt(a, s), ln(s, a, e, n, r);
  const c = await ze(s, r), l = Ve(e, n), d = await tn(l, s, {
    config: a,
    script: c,
    textureUrlResolver: async (g) => {
      const y = `${s}/${g.replace(/^\.\//, "")}`;
      return l.resolveImageUrl(y);
    }
  });
  return a != null && a.title || (d.meta.title = He(s)), d;
}
async function ln(i, e, t, n, r) {
  const s = Ve(t, n), o = await ze(i, r);
  let a = null;
  const c = `${i}/common.glsl`;
  B(t, c) && (a = await s.readText(c));
  const l = [], u = [
    { kind: "none" },
    { kind: "none" },
    { kind: "none" },
  ...
  for (const d of e.views) {
    const g = `${i}/${d}.glsl`, y = `${i}/${d}/image.glsl`;
    let T;
    if (B(t, g))
      T = await s.readText(g);
    else if (B(t, y))
...
const un = /* @__PURE__ */ Object.assign({ "./shaders/packing/common.glsl": () => Promise.resolve().then(() => Tn).then((i) => i.default), "./shaders/packing/image.glsl": () => Promise.resolve().then(() => xn).then((i) => i.default) }), dn = /* @__PURE__ */ Object.assign({ "./shaders/packing/config.json": () => Promise.resolve().then(() => _n).then((i) => i.default) }), hn = /* @__PURE__ */ Object.assign({}), mn = /* @__PURE__ */ Object.assign({ "./shaders/packing/script.js": () => Promise.resolve().then(() => Rn) });
let se = null;
async function pn() {
  return se || (se = await cn("shaders/packing", un, dn, hn, mn)), se;
}
async function Sn(i, e = {}) {
  const t = await pn();
  return Ot(i, { project: t, ...e });
}
class fn {
  constructor(e, t) {
  ...
  async showTab(e) {
    ...
    if (this.tabBar.querySelectorAll(".editor-tab-button").forEach((n, r) => {
      ...
      try {
        const { createEditor: s } = await Promise.resolve().then(() => Xe);
...
var Ae = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {}, je = { exports: {} };
(function(i) {
  ...
  var t = function(n) {
    var r = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, s = 0, o = {}, a = {
      ...
      util: {
        ...
        clone: function m(h, f) {
          ...
          switch (a.util.type(h)) {
            case "Object":
              if (v = a.util.objId(h), f[v])
                return f[v];
              p = /** @type {Record<string, any>} */
              {}, f[v] = p;
              for (var w in h)
                h.hasOwnProperty(w) && (p[w] = m(h[w], f));
              return (
                /** @type {any} */
                p
              );
            case "Array":
              return v = a.util.objId(h), f[v] ? f[v] : (p = [], f[v] = p, /** @type {Array} */
              /** @type {any} */
              h.forEach(function(R, b) {
                p[b] = m(R, f);
              }), /** @type {any} */
              p);
            default:
        ...
        insertBefore: function(m, h, f, p) {
          ...
          for (var R in v)
            if (v.hasOwnProperty(R)) {
              if (R == h)
                for (var b in f)
                  f.hasOwnProperty(b) && (w[b] = f[b]);
              f.hasOwnProperty(R) || (w[R] = v[R]);
            }
          var C = p[m];
          return p[m] = w, a.languages.DFS(a.languages, function(F, S) {
            S === C && F != m && (this[F] = w);
          }), w;
        },
        // Traverse a language definition with Depth First Search
        DFS: function m(h, f, p, v) {
          v = v || {};
          var w = a.util.objId;
          for (var R in h)
            if (h.hasOwnProperty(R)) {
              f.call(h, R, h[R], p || R);
              var b = h[R], C = a.util.type(b);
              C === "Object" && !v[w(b)] ? (v[w(b)] = !0, m(b, f, null, v)) : C === "Array" && !v[w(b)] && (v[w(b)] = !0, m(b, f, R, v));
            }
        }
      },
      plugins: {},
      ...
      highlightAllUnder: function(m, h, f) {
        ...
        for (var v = 0, w; w = p.elements[v++]; )
          a.highlightElement(w, h === !0, p.callback);
      },
      /**
      ...
      highlightElement: function(m, h, f) {
        ...
        function C(S) {
          b.highlightedCode = S, a.hooks.run("before-insert", b), b.element.innerHTML = b.highlightedCode, a.hooks.run("after-highlight", b), a.hooks.run("complete", b), f && f.call(b.element);
        }
        if (a.hooks.run("before-sanity-check", b), w = b.element.parentElement, w && w.nodeName.toLowerCase() === "pre" && !w.hasAttribute("tabindex") && w.setAttribute("tabindex", "0"), !b.code) {
          a.hooks.run("complete", b), f && f.call(b.element);
          return;
        }
        if (a.hooks.run("before-highlight", b), !b.grammar) {
          C(a.util.encode(b.code));
          return;
        }
        if (h && n.Worker) {
          ...
          C(a.highlight(b.code, b.grammar, b.language));
      },
      /**
      ...
      tokenize: function(m, h) {
        ...
        var v = new d();
        return g(v, v.head, m), u(m, v, h, v.head, 0), T(v);
      },
      /**
       * @namespace
       * @memberof Prism
    ...
    c.stringify = function m(h, f) {
      ...
      w && (Array.isArray(w) ? Array.prototype.push.apply(v.classes, w) : v.classes.push(w)), a.hooks.run("wrap", v);
      var R = "";
      for (var b in v.attributes)
        R += " " + b + '="' + (v.attributes[b] || "").replace(/"/g, "&quot;") + '"';
      ...
      for (var R in f)
        if (!(!f.hasOwnProperty(R) || !f[R])) {
          ...
          for (var C = 0; C < b.length; ++C) {
            if (w && w.cause == R + "," + C)
              return;
            var F = b[C], S = F.inside, I = !!F.lookbehind, he = !!F.greedy, We = F.alias;
            if (he && !F.pattern.global) {
              var qe = F.pattern.toString().match(/[imsuy]*$/)[0];
              F.pattern = RegExp(F.pattern.source, qe + "g");
            }
            for (var me = F.pattern || F, k = p.next, P = v; k !== h.tail && !(w && P >= w.reach); P += k.value.length, k = k.next) {
              var N = k.value;
              ...
              if (!(N instanceof c)) {
                var K = 1, L;
                if (he) {
                  if (L = l(me, P, m, I), !L || L.index >= m.length)
                    break;
                  var G = L.index, Ze = L.index + L[0].length, U = P;
                  for (U += k.value.length; G >= U; )
                    k = k.next, U += k.value.length;
                  if (U -= k.value.length, P = U, k.value instanceof c)
                    continue;
                  for (var z = k; z !== h.tail && (U < Ze || typeof z.value == "string"); z = z.next)
                    K++, U += z.value.length;
                  K--, N = m.slice(P, U), L.index -= P;
                } else if (L = l(me, 0, N, I), !L)
                  continue;
                var G = L.index, Y = L[0], te = N.slice(0, G), pe = N.slice(G + Y.length), ne = P + N.length;
                w && ne > w.reach && (w.reach = ne);
                var W = k.prev;
                te && (W = g(h, W, te), P += te.length), y(h, W, K);
                var Je = new c(R, S ? a.tokenize(Y, S) : Y, We, Y);
                if (k = g(h, W, Je), pe && g(h, k, pe), K > 1) {
                  var ie = {
                    cause: R + "," + C,
                    reach: ne
                  };
                  u(m, h, f, k.prev, P, ie), w && ie.reach > w.reach && (w.reach = ie.reach);
                }
              }
            }
          }
    ...
    function g(m, h, f) {
      var p = h.next, v = { value: f, prev: h, next: p };
      return h.next = v, p.prev = v, m.length++, v;
    }
    function y(m, h, f) {
      for (var p = h.next, v = 0; v < f && p !== m.tail; v++)
        p = p.next;
      h.next = p, p.prev = h, m.length -= v;
    }
    function T(m) {
    ...
    if (!n.document)
      return n.addEventListener && (a.disableWorkerMessageHandler || n.addEventListener("message", function(m) {
        var h = JSON.parse(m.data), f = h.language, p = h.code, v = h.immediateClose;
        n.postMessage(a.highlight(p, a.languages[f], f)), v && n.close();
      }, !1)), a;
    ...
    if (!a.manual) {
      ...
      _ === "loading" || _ === "interactive" && E && E.defer ? document.addEventListener("DOMContentLoaded", x) : window.requestAnimationFrame ? window.requestAnimationFrame(x) : window.setTimeout(x, 16);
    }
    return a;
  }(e);
  i.exports && (i.exports = t), typeof Ae < "u" && (Ae.Prism = t), t.languages.markup = {
    comment: {
      pattern: /<!--(?:(?!<!--)[\s\S])*?-->/,
      greedy: !0
    },
    ...
    var n = "Loading…", r = function(E, x) {
      return "✖ Error " + E + " while fetching file: " + x;
    ...
    t.hooks.add("before-highlightall", function(E) {
      ...
      if (x.matches(d)) {
        E.code = "", x.setAttribute(a, c);
        var _ = x.appendChild(document.createElement("CODE"));
        _.textContent = n;
        var m = x.getAttribute("data-src"), h = E.language;
        if (h === "none") {
        ...
        p && p.loadLanguages(h), g(
          ...
          function(v) {
            ...
            if (w) {
              var R = v.split(/\r\n?|\n/g), b = w[0], C = w[1] == null ? R.length : w[1];
              b < 0 && (b += R.length), b = Math.max(0, Math.min(b - 1, R.length)), C < 0 && (C += R.length), C = Math.max(0, Math.min(C, R.length)), v = R.slice(b, C).join(`
`), x.hasAttribute("data-start") || x.setAttribute("data-start", String(b + 1));
            }
            _.textContent = v, t.highlightElement(_);
          },
          function(v) {
            x.setAttribute(a, u), _.textContent = v;
          }
        );
      }
...
function En(i, e, t) {
  ...
  function l() {
    const g = o.value;
    c.textContent = g + `
`, vn.highlightElement(c);
    const y = g.split(`
`);
    r.innerHTML = y.map((T, E) => `<span>${E + 1}</span>`).join(""), t && t(g);
  }
  function u() {
  ...
  function d(g) {
    if (g.key === "Tab") {
      g.preventDefault();
      const y = o.selectionStart, T = o.selectionEnd, E = o.value;
      o.value = E.substring(0, y) + "  " + E.substring(T), o.selectionStart = o.selectionEnd = y + 2, l();
    }
  }
...
const Xe = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  createEditor: En
}, Symbol.toStringTag, { value: "Module" })), bn = `float EPS=1e-4;
...
`, xn = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: yn
}, Symbol.toStringTag, { value: "Module" })), Ke = "default", Ge = !0, Ye = {
  u_zoom: {
    type: "float",
    value: 1,
    hidden: !0
...
const ae = 5.5;
function ke(i, e, t, n) {
  const r = i.clientWidth / i.clientHeight, s = ae * 0.05 / D;
  O += e * s * r, $ += t * s, n.setUniformValue("u_pan", [O, $]);
}
function Cn(i) {
  requestAnimationFrame(() => {
    const e = document.querySelector("canvas");
    if (!e) return;
    const t = 96, n = 12, r = document.createElement("div");
    function s() {
      const l = e.getBoundingClientRect();
      r.style.left = `${l.right - n - t}px`, r.style.top = `${l.bottom - n - t}px`;
    }
    r.style.cssText = `
      position: fixed;
      display: grid;
    ...
    for (const [l, u, d, g, y] of c) {
      const T = document.createElement("button");
      if (T.textContent = d, T.style.cssText = `grid-column: ${g}; grid-row: ${y};` + a, d === "↺")
        T.addEventListener("mousedown", (E) => {
          E.stopPropagation(), D = 1, O = 0, $ = 0, i.setUniformValue("u_zoom", D), i.setUniformValue("u_pan", [O, $]);
        });
      else {
        let E = null;
        T.addEventListener("mousedown", (x) => {
          x.stopPropagation(), ke(e, l, u, i), E = setInterval(() => ke(e, l, u, i), 80);
    ...
    document.body.appendChild(r), e.addEventListener("wheel", (l) => {
      const u = l.deltaY > 0 ? 0.8695652173913044 : 1.15, d = D;
      D *= u, D = Math.max(1e-6, D);
      const g = e.getBoundingClientRect(), y = g.width / g.height, T = (l.clientX - g.left) / g.width, E = 1 - (l.clientY - g.top) / g.height, x = (T - 0.5) * y * ae, _ = (E - 0.5) * ae;
      O += x * (1 / d - 1 / D), $ += _ * (1 / d - 1 / D), i.setUniformValue("u_zoom", D), i.setUniformValue("u_pan", [O, $]), l.preventDefault();
    }, { passive: !1 });
  });
}
